...
- 当前正在阶段 06：已完成 `LineRenderer` 画圆和鼠标轨迹练习，准备进入范围检测。
- 最新练习包含 `Lesson12` 子弹预制体通过 `Resources.Load` 动态加载、`Lesson18_异步加载` 中的 `ResourcesMgr` 回调/协程两种异步加载写法、`Lesson20_场景异步加载` 中的 `SceneMgr` 异步切换场景封装，以及 `Lesson21_LineRender` 中的 `LineRenderer` 绘制练习。
- 下一步进入范围检测、射线检测和 46-68p 小项目实践；资源卸载边界后续继续复盘验证。
