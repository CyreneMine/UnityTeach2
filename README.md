...
- 当前正在阶段 05：已完成特殊文件夹创建、`Resources` 同步/异步加载练习和场景异步加载练习。
- 最新练习包含 `Lesson12` 子弹预制体通过 `Resources.Load` 动态加载、`Lesson18_异步加载` 中的 `ResourcesMgr` 回调/协程两种异步加载写法，以及 `Lesson20_场景异步加载` 中的 `SceneMgr` 异步切换场景封装。
- 下一步进入 `LineRenderer`、范围检测、射线检测和 46-68p 小项目实践；资源卸载边界后续继续复盘验证。
