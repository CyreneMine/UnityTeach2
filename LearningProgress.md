...
- 已识别视频：67 个
- 当前进度：基础知识主线（1-44p）与综合实践（46-68p）均已完成
- 当前阶段：阶段 09 - 实践总结与仓库复盘已完成
- 下一步：本系列学习项目收尾；后续可按需要发布 Release 或进入下一阶段课程

备注：课程编号 `45` 是知识点总结，当前不作为主线练习进度的关键节点。

实践技术路线说明：原教程小实践使用 `NGUI + XML`。考虑到 `NGUI` 已不是当前 Unity 项目的主流 UI 方案，且本项目希望继续练习现有的 JSON 数据流程，因此实际实现统一改为 `UGUI + JSON`。这里替换的是教程实践方案，不代表 XML 在所有开发场景中都已失去用途。
...
| 07 | 综合实践 UI 与数据准备 | 已完成 | 开始、设置、排行榜、选角和 GameScene 跳转已实现；UGUI 与 JSON 数据流程已验证 |
| 08 | 综合实践 gameplay 逻辑 | 已完成 | 玩家、子弹、开火点、生命/计时、退出/结束、排行榜写入和死亡音效已串联 |
| 09 | 实践总结、仓库复盘、可展示成果 | 已完成 | 已补齐最终复盘与当前状态；GitHub Release 保留为可选项 |
...
### 2026-07-12 - 46-68p 综合实践与基础课程项目收尾

- 完成 `46.需求分析.mp4` 到 `68.实践总结.mp4` 的综合实践，至此本课程基础知识与小实践主线全部结束。
- 实践继续采用 `UGUI + JSON` 替代教程的 `NGUI + XML`：`role.json`、`bullet.json`、`fire.json` 放入 `Assets/StreamingAssets/` 随构建发布；`music.json` 和 `rank.json` 保留在 `persistentDataPath` 作为可写数据。
- `BeginScene` 已完成开始、设置、排行榜、选角和退出入口；排行榜条目支持复用刷新，选角结果可以进入 `GameScene`。
- `GameScene` 使用 `Main` 根据当前角色数据生成玩家，应用速度和生命属性；游戏面板显示生命与存活时间，并提供退出确认和游戏结束返回流程。
- `FireObject` 在屏幕八个方向维护发射点，从 `FireData` 随机选择连续瞄准发射或扇形发射模式，并根据 `BulletData` 生成不同子弹。
- `BulletObject` 已实现直线、正弦横移、左右旋转和追踪等轨迹；玩家被子弹命中会扣血，鼠标射线命中 `Monster` 层子弹时可主动消除子弹。
- 游戏结束会记录姓名和存活时间并写入排行榜；设置面板的音效音量和开关会作用于子弹死亡音效。
- 死亡音效最终放到独立死亡特效对象上播放，避免子弹销毁时连同自身 `AudioSource` 一起终止声音。
- 本阶段重点排错包括：JSON 字段大小写必须与数据类一致；动态 UI 使用 `SetParent(parent, false)`；子弹扇形应旋转 `bullet.transform`；XZ 平面应绕 `Vector3.up` 展开；`Translate` 的向量必须和 `Space.Self/World` 匹配；玩家重复碰撞体会导致一次命中多次扣血。
- 当前五架飞机 Prefab 不自带 `PlayerObject`，由 `Main` 在实例化后动态添加，避免场景替换玩家时保留旧实例引用。
- 最终静态检查未发现最近的 Unity 编译或运行异常；课程成果以用户在 Editor 中的运行验证和当前磁盘文件为准。
- 新增笔记：`Notes/Practice46-68-Gameplay.md`。

### 2026-07-11 - 综合实践开始场景 UI 基础逻辑

- 明确本次实践的技术迁移：教程原方案为 `NGUI + XML`，当前项目改用 `UGUI + JSON` 实现相同的界面与数据目标；后续复盘以本仓库的实际实现为准，同时对照教程理解业务流程。
- 综合实践进入开始场景 UI 制作阶段，完成开始面板、设置面板和排行榜面板的基础逻辑。
- `BeginPanel` 已连接开始、排行榜、设置和退出按钮；开始按钮会隐藏开始面板并打开选角面板，选角确认后再进入 `GameScene`。
- `SettingPanel` 已连接音乐/音效音量和开关，并通过 `GameDataMgr` 读写设置数据；实际音效系统及游戏场景联动仍待后续完善。
- `RankPanel` 已从排行榜数据生成 `RankItem`，并使用 `ScrollRect` 的 Content 组织排行榜条目。
- 新增 `ChoosePanel` 与 `RoleData`：开始按钮进入选角面板，左右按钮切换飞机，10 个 Toggle 分别显示生命、速度和容量，鼠标拖动可旋转展示对象。
- 五架飞机 Prefab 已整理到 `Assets/Resources/Airplane/`；`BeginScene` 与新建的 `GameScene` 已加入 Build Settings，选角面板开始按钮可以切换到 `GameScene`。
- 排行榜条目改为在实例化时直接指定 Content 父节点，并将新条目加入 `rankItems`，避免重复打开面板时反复创建同一批条目。
...
### 2026-07-08 - 范围检测、射线检测与基础主线收尾

- 完成 `41.范围检测.mp4`、`42.范围检测  练习题.mp4`、`43.射线检测.mp4` 和 `44.射线检测  练习题.mp4`，基础知识主线 1-44p 按当前学习节奏收尾。
- 新增练习文件：`Assets/Scripts/Lesson/Lesson22_范围检测/Lesson22.cs`、`Assets/Scripts/Lesson/Lesson22_范围检测/Lesson22.unity`、`Assets/Scripts/Lesson/Lesson23_射线检测/Lesson23.cs`、`Assets/Scripts/Lesson/Lesson23_射线检测/Lesson23.unity`。
- `Lesson22` 实现键盘 `WASD` 控制物体前后移动和旋转，并通过 `J/K/L` 分别练习 `Physics.OverlapBox`、`Physics.OverlapCapsule` 和 `Physics.OverlapSphere`。
- `ProjectSettings/TagManager.asset` 新增 `Monster` 和 `Player` 层；范围检测使用 `Monster` 层过滤敌人，射线拖拽使用 `Player` 层选中对象、`Default` 层命中平面。
- 范围检测检查中确认：胶囊检测端点已改为基于 `transform.position` 的世界坐标点，避免检测范围脱离玩家位置。
- `Lesson23` 实现鼠标点击墙面生成命中特效和弹孔，并实现点击选中场景物体后按住左键在平面上拖动，右键取消选中。
- 新增资源：`Assets/Resources/Effect/HitEff.prefab`、`Assets/Resources/Effect/DDD.prefab` 以及 `Assets/ArtRes/BulletEff/` 下的特效资源。
- 记录边界：当前左键同时承载“打墙特效”和“选中拖拽”两类练习逻辑，特效朝向也没有按 `hit.normal` 做项目化处理；这些在当前练习场景中可接受，正式项目中再拆输入状态和命中表现。
- 新增笔记：`Notes/Lesson41-44-PhysicsDetection.md`。
- 阶段 06 已收尾；`45` 为知识点总结，不作为新的主线练习节点。下一步进入 `46.需求分析.mp4` 到 `68.实践总结.mp4` 的综合实践项目。
